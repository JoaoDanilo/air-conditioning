...
void mkl_GPIO::bindPeripheral(uint8_t GPIONumber, uint8_t pinNumber) {
  ...
  /*!
     * C�lculo do endere�o absoluto do ISFR para o GPIO.
     * Address(hexa): GPIOA=400FF014 B=400FF054 C=400FF094 D=400FF0D4 E=400FF114.
     * addressPortxISFR = 0x40049000 (Base) + 0x1000*(0,1,2,3,4 ou 5)
     *                    + 4*(0,1,2,3,4,...,30) (Offset).
     */
  addressPortxISFR = (volatile uint32_t *)(0x40049000 + 0xA0);
}
 ...
 *
 */
void mkl_GPIO::setGPIOParameters(gpio_Pin pin, uint32_t &gpio, uint32_t &pinNumber) {
  pinNumber = pin & 0xFF;
  gpio = pin >> 8;
  pinPort = 1 << pin;
}
